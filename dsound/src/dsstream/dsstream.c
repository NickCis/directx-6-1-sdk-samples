...
int StreamBufferSetup( void )
    ...
    DPF3(0, "BufferSize = %lu ", wiWave.dwBufferSize);
...
int SetupNotifications(void)
{
	...
	DPF3(0, "Set notifies for position %lu", dsbPosNotify[0].dwOffset);
	...
	for (i = 1; i < NUM_PLAY_NOTIFICATIONS; i++)
	{
		...
		DPF3(0, "Set notifies for positions %lu", dsbPosNotify[i].dwOffset);
	}
