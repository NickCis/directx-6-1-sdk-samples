		...
		case 0:
			{	// A play notification has been received.
				...
				{	// read in more of the file.
					DPF3(0, "Received a notify event with FoundEnd = FALSE. NR = %lu", wiWave.dwNextWriteOffset );
					...
					DPF3(1, "dwPlayPos = %lu", dwPlayPos);
					DPF3(1, "dwWritePos = %lu", dwWritePos);
#endif					
					// Lock the buffer.
					...
					if (hr != DS_OK)
					{
						...
						DPF3(0, "couldn't lock buffer with hr = %lu", hr);
						return hr;
					}
					...
					{
						DPF3(3, "ReadFile read less than what we asked for. less by %lu", dwWrite1-(DWORD)cbActual);
						if (!wiWave.bLoopFile)
						{
...
void UpdateProgressBar(void)
{
	...
	if (lpDSBStreamBuffer)
	{
		...
		DPF3(1, "CurrentPos = %lu", dwPlayPos);
		DPF3(1, "LastPos = %lu", wiWave.dwLastPos);
		...
		DPF3(1, "wiWave.dwProgress =	 %lu", wiWave.dwProgress);
		DPF3(1, "Posting %lu", wiWave.dwProgress); 
