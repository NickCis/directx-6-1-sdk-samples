...
BOOL InitializeDSound( void )
	...
	{
	DPF3( 0, "Could not create a DSound object (%s)", TranslateDSError(hr));
    ...
    if (FAILED(hr))
	{
	DPF3( 0, "Failed Initialize() on IDirectSound object (%s)", TranslateDSError(hr));
    ...
    if (FAILED(hr))
	{
	DPF3( 0, "Couldn't SetCooperativeLevel() (%s)", TranslateDSError(hr));
...
BOOL InitPrimarySoundBuffer( void )
    ...
    DPF3( 2, "Initial format code: %lu", gdwOutputFormat );
	...
	{
	DPF3( 0, "Couldn't create primary buffer (%s)", TranslateDSError(hr));
	goto IPSB_ExitError;
	}
    ...
    if( FAILED( hr = gpdsbPrimary->SetFormat( gpwfxFormat )))
	{
		...
		nCurFormat = 0;
		DPF3( 2, "Unable to SetFormat() preferred format to %lu", gdwOutputFormat );

		while( FAILED( hr ) && nCurFormat < NUM_FORMATENTRIES )
		{
			gdwOutputFormat = aFormatOrder[nCurFormat];
			FormatCodeToWFX( gdwOutputFormat, gpwfxFormat );
			DPF3( 2, "Trying format code: %lu", gdwOutputFormat );

			hr = gpdsbPrimary->SetFormat( gpwfxFormat );
			if( FAILED(hr))
			{
				DisableFormatCode( gdwOutputFormat );
				DPF3( 2, "Return from SetFormat on primary buffer = %s", TranslateDSError(hr));
			}
			else
			{
				EnableFormatCode( gdwOutputFormat );
				DPF3( 2, "Succeeded on SetFormat() for code %lu", gdwOutputFormat );
			}
    ...
    if( FAILED( hr = gpdsbPrimary->QueryInterface( IID_IDirectSound3DListener,
						(void**)&gp3DListener)))
	{
	DPF3( 0, "Couldn't QI primary buffer for 3D listener interface (%s)", TranslateDSError(hr));
	goto IPSB_ExitError;
	}
    if( FAILED( hr = gpdsbPrimary->Play( 0, 0, DSBPLAY_LOOPING )))
	{
	DPF3( 0, "Couldn't play primary buffer (%s)", TranslateDSError(hr));
