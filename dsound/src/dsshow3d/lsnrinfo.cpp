...
BOOL ListenerInfo::OnCommand( WPARAM wParam, LPARAM lParam )
	...
	if( (HWND)lParam == m_hDistanceEdit )
	    ...
	    DPF3( 3, "Setting Distance Factor: %s", szEdit );
	    ...
	    ASSERT( NULL != gp3DListener );
	    DPF3( 3, "Setting Doppler Factor: %s", szEdit );
	    ...
	    ASSERT( NULL != gp3DListener );
	    DPF3( 3, "Setting Rolloff Factor: %s", szEdit );
	    ...
	    ASSERT( NULL != gp3DListener );
	    DPF3( 3, "Setting Front X: %s", szEdit );
	    ...
	    ASSERT( NULL != gp3DListener );
	    DPF3( 3, "Setting Front Y: %s", szEdit );
	    ...
	    ASSERT( NULL != gp3DListener );
	    DPF3( 3, "Setting Front Z: %s", szEdit );
...
void ListenerInfo::HandleXSliderScroll( WORD wNot, LONG lPos )
	...
	DPF5( 3, "Setting listener pos: (%i, %i, %i)", (int)m_vPos.x, (int)m_vPos.y, (int)m_vPos.z );
	if( FAILED( hr = gp3DListener->SetPosition( m_vPos.x, m_vPos.y,
						m_vPos.z, DS3D_IMMEDIATE )))
	    DPF3( 0, "IDirectSound3DListener::SetPosition returned : %s", TranslateDSError(hr));
