...
void WINAPI DbgDumpWFX( int nLevel, PWAVEFORMATEX pwfx )
    {
    DPF3( nLevel, "Dumping WAVEFORMATEX at 0x%08lX", pwfx );
    DPF5( nLevel, "wFormatTag = %u, nChannels = %u, nSamplesPerSec = %lu",
		pwfx->wFormatTag, pwfx->nChannels, pwfx->nSamplesPerSec );
    DPF5( nLevel, "nAvgBytesPerSec = %lu, nBlockAlign = %u, wBitsPerSample = %lu",
		pwfx->nAvgBytesPerSec, pwfx->nBlockAlign, pwfx->wBitsPerSample );
    DPF3( nLevel, "cbSize = %u",pwfx->cbSize );
    }
