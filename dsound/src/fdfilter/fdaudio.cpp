...
static BOOL InitInputDevice( void )
	...
	else
	{
		...
		{
			DPF3( 0, "Couldn't open DirectSoundCapture device (%s)", TranslateDSError(dsrval));
			goto IID_ExitError;
		}
	}
	return TRUE;
...
static BOOL InitPrimarySoundBuffer( void )
	...
	else
	{
		...
		{
			DPF3( 0, "Couldn't open DirectSound device (%s)", TranslateDSError(dsrval));
			goto IPSB_ExitError;
		}
	}
	...
	{
	DPF3( 0, "Couldn't get PRIORITY cooperative level (%s)", TranslateDSError(dsrval));
	goto IPSB_ExitError;
	}

    if( FAILED( dsrval = gpds->CreateSoundBuffer( &dsbd, &gpdsbPrimary, NULL )))
	{
	DPF3( 0, "Couldn't create primary buffer (%s)", TranslateDSError(dsrval));
	goto IPSB_ExitError;
	}
...
void ScanAvailableDSFormats( void )
    ...
    DPF3( 3, "Scanning %u DirectSound formats for availability", NUM_FORMATCODES );
	...
	if( FAILED( dsrval = gpdsbPrimary->SetFormat( &wfx )))
	    {
	    DPF3( 5, "Failed with SetFormat() for %u format", aOutputFormats[i].dwCode );
	    aOutputFormats[i].fEnabled = FALSE;
	    }
	else
	    {
	    DPF3( 5, "Succeeded with SetFormat() for %u format", aOutputFormats[i].dwCode );
...
void ScanAvailableDSCFormats( void )
    ...
    DPF3( 3, "Scanning %u DirectSoundCapture formats for availability", NUM_FORMATCODES );
    if( NULL == gpdsc )
	{
	for( i = 0; i < NUM_FORMATCODES; i++ )
	...
	if ((dsrval = gpdsc->CreateCaptureBuffer(&dscbd, &pdscb, NULL)))
	    {
	    DPF3( 5, "Failed with DirectSoundCapture::CreateCaptureBuffer() for %u format", aInputFormats[i].dwCode );
	    aInputFormats[i].fEnabled = FALSE;
	    }
	else
	    {
	    DPF3( 5, "Succeeded with DirectSoundCapture::CreateCaptureBuffer() for %u format", aInputFormats[i].dwCode );
	    pdscb->Release();
...
BOOL InitBuffers( void )
	...
	{
	DPF3( 0, "Unable to QI for IDirectSoundNotify on input (%s)", TranslateDSError(dsrval));
	...
	if( FAILED( dsrval ) )
	{
	DPF3( 0, "Unable to SetNotificationPositions on input (%s)", TranslateDSError(dsrval));
	goto Abort_InitBuffers;
	}
	...
	{
	DPF3( 0, "Unable to create sound buffer for output (%s)", TranslateDSError(dsrval));
	goto Abort_InitBuffers;
	}
...
static void ProcessInput( DWORD iEvent )
{
	...
	if( FAILED(dsrval) )
	{
		DPF3( 0, "Couldn't Lock input buffer. (%s)", TranslateDSError(dsrval));
		goto WIC_SkipDataProcessing;
	}
	...
	if( NULL != gpdsbOutput )
	{
		...
		{
			DPF3( 0, "Unable to get buffer status. (%s)", TranslateDSError(dsrval));
			goto WIC_SkipDataProcessing;
		}
		if( dwStatus & DSBSTATUS_BUFFERLOST )
		{
			...
			{
				DPF3( 0, "Couldn't Restore output buffer. (%s)", TranslateDSError(dsrval));
				goto WIC_SkipDataProcessing;
			}
			dwWritePosition = 0;
		...
		if( fRestoredBuffer )
		{
			...
			if( FAILED( dsrval = gpdsbOutput->Play( 0, 0, DSBPLAY_LOOPING )))
				DPF3( 0, "Unable to restart restored buffer. (%s)", TranslateDSError(dsrval));
			dwWritePosition = 0xFFFFFFFF;
		}
	}
