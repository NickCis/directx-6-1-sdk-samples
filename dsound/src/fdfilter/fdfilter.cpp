	...
	case WM_COMMAND:
		...
		case IDC_FORMATS_OUTPUT_LISTBOX:
			...
			if( FAILED( dsrval = gpdsbPrimary->SetFormat( gpwfxOutput )))
			    {
			    DPF4( 1, "Formats DLG: Failed to set DSound primary to %lu (%s)",
			    ...
			    if( FAILED( dsrval = gpdsbPrimary->SetFormat( gpwfxOutput )))
				{
				DPF4( 0, "Formats DLG: Failed to set OLD format (%lu) on primary (%s)",
		...
		case IDOK:
...

			{
				...
				DPF3( 1, "Set input buffer size to %lu bytes", gcbBufferSize );
				...
				{
				DPF4( 0, "Unable to open selected input device! format = %lu, dsrval = %lu", dwInFormat, dsrval );
				goto Abort_OK;
				}
				else
				{
