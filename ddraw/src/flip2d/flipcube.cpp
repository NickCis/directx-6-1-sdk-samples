...
BOOL CreateMouse(GUID &guid, LPDIRECTINPUTDEVICE& lpdiMouse, DWORD dwAccess)
{
    ...
    if(err != DI_OK)
    {
            if(dd != NULL)
                dd->FlipToGDISurface();
            MessageBox(hwndApp, "Unable to Create DirectInput Mouse Device",
    ...
    if(err != DI_OK)
    {
            if(dd != NULL)
                dd->FlipToGDISurface();
            MessageBox(hwndApp, "Unable to Access DirectInput Device as a mouse",
                    "DirectDraw Spinning Cube", MB_OK);
            goto fail;
    }
    ...
    if(err != DI_OK)
    {
        ...
        if (dwAccess & DISCL_EXCLUSIVE) {
            if(dd != NULL)
                dd->FlipToGDISurface();
            MessageBox(hwndApp, "Unable to set cooperativity level",
                    "DirectDraw Spinning Cube", MB_OK);
        }
        goto fail;
    }
...
BOOL InitDInput(void)
{
    ...
    if(err != DI_OK)
    {
        if(dd != NULL)
            dd->FlipToGDISurface();
        MessageBox(hwndApp, "Unable to Create DirectInput Object",
            "DirectDraw Spinning Cube", MB_OK);
        return FALSE;
    }
