...
#define DEPTH          0.80f  // How close the camera can get to the end of
                              // track before new segments are added.
#define PATH_LENGTH   25.00f  // Total length of the tunnel before it wraps
...
HRESULT App_OneTimeSceneInit( HWND hWnd )
{
    ...
    {
        FLOAT t = (2.0f*g_PI*i)/NUM_SPLINE_POINTS;
        g_Tunnel.pvPoints[i].x = 20.0f * (FLOAT)cos( t * 32.0f );
        g_Tunnel.pvPoints[i].y = 20.0f * (FLOAT)sin( t * 32.0f );
        g_Tunnel.pvPoints[i].z = 20.0f * (FLOAT)cos( t *  2.0f );
    }
